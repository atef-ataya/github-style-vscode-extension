...
class PatternAnalyzer {
  ...
  extractFunctions(content) {
    ...
    const functionRegex =
      /(?:function\s+([\w$]{1,100})|(?:const|let|var)\s+([\w$]{1,100})\s*=\s*(?:async\s*)?function|(?:const|let|var)\s+([\w$]{1,100})\s*=\s*(?:async\s*)?\([^)]\)\s*=>)\s*\{([^}]{0,5000})\}/g;
    const functions = [];
    let match;
  ...
  extractClasses(content) {
    ...
    while (
      (match = classRegex.exec(content)) !== null &&
      iterationCount < MAX_ITERATIONS
    ) {
      iterationCount++;
      const name = match[1];
  ...
  detectVariableNamingStyle(content) {
    const camelCaseRegex =
      /(?:let|const|var)\s+[a-z][a-zA-Z0-9]*[A-Z][a-zA-Z0-9]*/;
  ...
  detectClassNamingStyle(content) {
    ...
    return (content.match(pascalCaseRegex) || []).length > 0
      ? 'PascalCase'
      : 'unknown';
  }
  ...
  extractImports(content) {
    ...
    // Simplified regex with limited backtracking
    const importRegex =
      /import\s+(?:{[^}]{0,500}}\s+from\s+)?['"]([@\w-]{1,200})['"]\.?/g;
    let match;
    // Set a limit on iterations to prevent infinite loops
    let iterationCount = 0;
    const MAX_ITERATIONS = 1000;
  ...
  identifyFrameworks(imports) {
    ...
    return imports.filter(imp =>
      commonFrameworks.some(framework =>
        imp.toLowerCase().includes(framework.toLowerCase())
      )
    );
  }
  identifyLibraries(imports) {
    return imports.filter(
      imp => !this.identifyFrameworks([imp]).length && !imp.startsWith('.')
    );
  }
